      ...
      <div>
        ...
        {weeks.map((week, weekIndex) => {
          ...
          const highlightWholeWeek =
            isDateInSelectedWeek(week.monday) || (isRangeMode && isEndDateInSelectedWeek(week.monday));
  
          return (
            <div key={weekIndex} className="mb-3">
              <div
                className={`text-xs mb-1 px-2 py-1 rounded font-medium ${
                  highlightWholeWeek ? 'bg-blue-100 text-blue-700' : 'text-gray-500'
                }`}
              >
                第{weekNum}周 {weekRange.monday.getMonth() + 1}/{weekRange.monday.getDate()} - {weekRange.sunday.getMonth() + 1}/{weekRange.sunday.getDate()}
              </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {months.map((month, index) => {
          ...
          const isSelected = selectedDate && 
            ...
            selectedDate.getFullYear() === currentYear;
          const isRangeStart = isRangeMode && selectedDate && 
            selectedDate.getMonth() === index && 
            selectedDate.getFullYear() === currentYear;
          const isRangeEnd = isRangeMode && selectedEndDate && 
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {years.map((year) => {
            ...
            const isInRange = isRangeMode && selectedDate && selectedEndDate &&
              year >= selectedDate.getFullYear() && year <= selectedEndDate.getFullYear();
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-md mx-auto space-y-4">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">移动端日期选择器</h1>
        ...
        >
          <span className={selectedDate || (isRangeMode && selectedEndDate) ? 'text-gray-800' : 'text-gray-400'}>
            {formatRangeDisplay()}
          </span>
      ...
      {showPicker && (
        <div className="fixed inset-0 z-40" onClick={() => setShowPicker(false)}>
          ...
          {/* 弹出内容 */}
          <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl z-50 animate-slide-up" onClick={(e) => e.stopPropagation()}>
            <div className="p-4">
              {/* 头部 */}
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-800">
                  {isRangeMode ? '选择范围' : '选择'}
                  {pickerType === 'day' ? '日期' : pickerType === 'week' ? '周' : pickerType === 'month' ? '月份' : pickerType === 'quarter' ? '季度' : '年份'}
                </h3>
                <button
